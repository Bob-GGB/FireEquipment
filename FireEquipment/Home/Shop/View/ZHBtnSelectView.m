...
- (instancetype)initWithFrame:(CGRect)frame titles:(NSArray *)titles catIDnum:(NSArray *)catNumArr column:(NSInteger)column {
    if (self = [super initWithFrame:frame]) {
        self.btnTitles = titles;
        self.column = column;
        self.catIDnums=catNumArr;
        [self setupContent:titles catIDnum:catNumArr column:column];
    }
    return self;
}

- (void)setupContent:(NSArray *)titles catIDnum:(NSArray *)catNumArr column:(NSInteger)column {
    ...
    for (int i = 0; i < titles.count; i++) {
        ...
        btn.selectCatIDNum= catNumArr[i];
