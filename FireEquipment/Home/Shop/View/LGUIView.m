...
- (instancetype)initWithFrame:(CGRect)frame indexArray:(NSArray *)array
{
    ...
    if (self)
    {
        ...
        {
            _defaultLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, i * hh, self.frame.size.width, hh)];
            _defaultLabel.text = self.indexArray[i];
            _defaultLabel.tag = TAG + i;
            _defaultLabel.textAlignment = NSTextAlignmentCenter;
            _defaultLabel.textColor = [UIColor colorWithHexString:@"#b6463b"];
            _defaultLabel.font = FONT_SIZE;
//            label.backgroundColor=[UIColor greenColor];
            [self addSubview:_defaultLabel];
            
            _number = _defaultLabel.font.pointSize;
        
        }
        if (array.lastObject) {
            self.ViewHeigth=CGRectGetMaxY(_defaultLabel.frame);
        }
...
-(UILabel *)animationLabel
{
    if (!_animationLabel)
    {
        ...
        _animationLabel.backgroundColor = [UIColor colorWithHexString:@"#b6463b"];
...
-(void)panAnimationFinish
{
    ...
    for (int i = 0; i < self.indexArray.count; i ++)
    {
        ...
        [UIView animateWithDuration:0.2 animations:^{
            ...
            label.alpha = 1.0;
            label.textColor = [UIColor colorWithHexString:@"#b6463b"];
        }];
    }
